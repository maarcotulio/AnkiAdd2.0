export const wordMock = [
  {
    word: "mother",
    phonetics: [
      {
        audio:
          "https://api.dictionaryapi.dev/media/pronunciations/en/mother-au.mp3",
        sourceUrl: "https://commons.wikimedia.org/w/index.php?curid=75833745",
        license: {
          name: "BY-SA 4.0",
          url: "https://creativecommons.org/licenses/by-sa/4.0",
        },
      },
      {
        text: "/ˈmʌðə(ɹ)/",
        audio:
          "https://api.dictionaryapi.dev/media/pronunciations/en/mother-uk.mp3",
        sourceUrl: "https://commons.wikimedia.org/w/index.php?curid=813823",
      },
      {
        text: "/ˈmʌðɚ/",
        audio:
          "https://api.dictionaryapi.dev/media/pronunciations/en/mother-us.mp3",
        sourceUrl: "https://commons.wikimedia.org/w/index.php?curid=1113599",
        license: {
          name: "BY-SA 3.0",
          url: "https://creativecommons.org/licenses/by-sa/3.0",
        },
      },
    ],
    meanings: [
      {
        partOfSpeech: "noun",
        definitions: [
          {
            definition: "A (human) female who has given birth to a baby",
            synonyms: [],
            antonyms: [],
            example: "He had something of his mother in him.",
          },
          {
            definition:
              "A human female who parents an adopted or fostered child",
            synonyms: [],
            antonyms: [],
          },
          {
            definition:
              "A human female who donates a fertilized egg or donates a body cell which has resulted in a clone.",
            synonyms: [],
            antonyms: [],
          },
          {
            definition:
              "A pregnant female, possibly as a shortened form of mother-to-be.",
            synonyms: [],
            antonyms: [],
            example:
              "Nutrients and oxygen obtained by the mother are conveyed to the fetus.",
          },
          {
            definition: "A female parent of an animal.",
            synonyms: [],
            antonyms: [],
            example: "The lioness was a mother of four cubs.",
          },
          {
            definition: "A female ancestor.",
            synonyms: [],
            antonyms: [],
          },
          {
            definition: "A source or origin.",
            synonyms: [],
            antonyms: [],
            example:
              "The Mediterranean was mother to many cultures and languages.",
          },
          {
            definition:
              "Something that is the greatest or most significant of its kind. (See mother of all.)",
            synonyms: [],
            antonyms: [],
          },
          {
            definition:
              "(when followed by a surname) A title of respect for one's mother-in-law.",
            synonyms: [],
            antonyms: [],
            example: "Mother Smith, meet my cousin, Doug Jones.",
          },
          {
            definition:
              "Any elderly woman, especially within a particular community.",
            synonyms: [],
            antonyms: [],
          },
          {
            definition: "Any person or entity which performs mothering.",
            synonyms: [],
            antonyms: [],
          },
          {
            definition:
              "The principal piece of an astrolabe, into which the others are fixed.",
            synonyms: [],
            antonyms: [],
          },
          {
            definition:
              "The female superior or head of a religious house; an abbess, etc.",
            synonyms: [],
            antonyms: [],
          },
          {
            definition: "Hysterical passion; hysteria; the uterus.",
            synonyms: [],
            antonyms: [],
          },
        ],
        synonyms: ["father", "granddaddy", "grandfather", "metro-"],
        antonyms: ["child", "daughter", "offspring", "son", "father"],
      },
    ],
    license: {
      name: "CC BY-SA 3.0",
      url: "https://creativecommons.org/licenses/by-sa/3.0",
    },
    sourceUrls: ["https://en.wiktionary.org/wiki/mother"],
  },
  {
    word: "mother",
    phonetics: [
      {
        audio:
          "https://api.dictionaryapi.dev/media/pronunciations/en/mother-au.mp3",
        sourceUrl: "https://commons.wikimedia.org/w/index.php?curid=75833745",
        license: {
          name: "BY-SA 4.0",
          url: "https://creativecommons.org/licenses/by-sa/4.0",
        },
      },
      {
        text: "/ˈmʌðə(ɹ)/",
        audio:
          "https://api.dictionaryapi.dev/media/pronunciations/en/mother-uk.mp3",
        sourceUrl: "https://commons.wikimedia.org/w/index.php?curid=813823",
      },
      {
        text: "/ˈmʌðɚ/",
        audio:
          "https://api.dictionaryapi.dev/media/pronunciations/en/mother-us.mp3",
        sourceUrl: "https://commons.wikimedia.org/w/index.php?curid=1113599",
        license: {
          name: "BY-SA 3.0",
          url: "https://creativecommons.org/licenses/by-sa/3.0",
        },
      },
    ],
    meanings: [
      {
        partOfSpeech: "verb",
        definitions: [
          {
            definition:
              "To give birth to or produce (as its female parent) a child. (Compare father.)",
            synonyms: [],
            antonyms: [],
          },
          {
            definition:
              "To treat as a mother would be expected to treat her child; to nurture.",
            synonyms: [],
            antonyms: [],
          },
        ],
        synonyms: [],
        antonyms: [],
      },
    ],
    license: {
      name: "CC BY-SA 3.0",
      url: "https://creativecommons.org/licenses/by-sa/3.0",
    },
    sourceUrls: ["https://en.wiktionary.org/wiki/mother"],
  },
  {
    word: "mother",
    phonetics: [
      {
        audio:
          "https://api.dictionaryapi.dev/media/pronunciations/en/mother-au.mp3",
        sourceUrl: "https://commons.wikimedia.org/w/index.php?curid=75833745",
        license: {
          name: "BY-SA 4.0",
          url: "https://creativecommons.org/licenses/by-sa/4.0",
        },
      },
      {
        text: "/ˈmʌðə(ɹ)/",
        audio:
          "https://api.dictionaryapi.dev/media/pronunciations/en/mother-uk.mp3",
        sourceUrl: "https://commons.wikimedia.org/w/index.php?curid=813823",
      },
      {
        text: "/ˈmʌðɚ/",
        audio:
          "https://api.dictionaryapi.dev/media/pronunciations/en/mother-us.mp3",
        sourceUrl: "https://commons.wikimedia.org/w/index.php?curid=1113599",
        license: {
          name: "BY-SA 3.0",
          url: "https://creativecommons.org/licenses/by-sa/3.0",
        },
      },
    ],
    meanings: [
      {
        partOfSpeech: "noun",
        definitions: [
          {
            definition:
              "A stringy, mucilaginous or film- or membrane-like substance (consisting of acetobacters) which develops in fermenting alcoholic liquids (such as wine, or cider), and turns the alcohol into acetic acid with the help of oxygen from the air.",
            synonyms: [],
            antonyms: [],
            example: "pieces of mother, adding mother to vinegar",
          },
        ],
        synonyms: [],
        antonyms: [],
      },
      {
        partOfSpeech: "verb",
        definitions: [
          {
            definition: "To cause to contain mother.",
            synonyms: [],
            antonyms: [],
            example: "mothered oil / vinegar / wine",
          },
          {
            definition: "(of an alcohol) To develop mother.",
            synonyms: [],
            antonyms: [],
          },
        ],
        synonyms: [],
        antonyms: [],
      },
    ],
    license: {
      name: "CC BY-SA 3.0",
      url: "https://creativecommons.org/licenses/by-sa/3.0",
    },
    sourceUrls: ["https://en.wiktionary.org/wiki/mother"],
  },
  {
    word: "mother",
    phonetics: [
      {
        audio:
          "https://api.dictionaryapi.dev/media/pronunciations/en/mother-au.mp3",
        sourceUrl: "https://commons.wikimedia.org/w/index.php?curid=75833745",
        license: {
          name: "BY-SA 4.0",
          url: "https://creativecommons.org/licenses/by-sa/4.0",
        },
      },
      {
        text: "/ˈmʌðə(ɹ)/",
        audio:
          "https://api.dictionaryapi.dev/media/pronunciations/en/mother-uk.mp3",
        sourceUrl: "https://commons.wikimedia.org/w/index.php?curid=813823",
      },
      {
        text: "/ˈmʌðɚ/",
        audio:
          "https://api.dictionaryapi.dev/media/pronunciations/en/mother-us.mp3",
        sourceUrl: "https://commons.wikimedia.org/w/index.php?curid=1113599",
        license: {
          name: "BY-SA 3.0",
          url: "https://creativecommons.org/licenses/by-sa/3.0",
        },
      },
    ],
    meanings: [
      {
        partOfSpeech: "noun",
        definitions: [
          {
            definition: "Motherfucker.",
            synonyms: [],
            antonyms: [],
          },
          {
            definition: "A striking example.",
            synonyms: [],
            antonyms: [],
          },
        ],
        synonyms: ["MF", "mofo", "motherfucker", "mutha"],
        antonyms: [],
      },
    ],
    license: {
      name: "CC BY-SA 3.0",
      url: "https://creativecommons.org/licenses/by-sa/3.0",
    },
    sourceUrls: ["https://en.wiktionary.org/wiki/mother"],
  },
  {
    word: "mother",
    phonetics: [
      {
        audio:
          "https://api.dictionaryapi.dev/media/pronunciations/en/mother-au.mp3",
        sourceUrl: "https://commons.wikimedia.org/w/index.php?curid=75833745",
        license: {
          name: "BY-SA 4.0",
          url: "https://creativecommons.org/licenses/by-sa/4.0",
        },
      },
      {
        text: "/ˈmʌðə(ɹ)/",
        audio:
          "https://api.dictionaryapi.dev/media/pronunciations/en/mother-uk.mp3",
        sourceUrl: "https://commons.wikimedia.org/w/index.php?curid=813823",
      },
      {
        text: "/ˈmʌðɚ/",
        audio:
          "https://api.dictionaryapi.dev/media/pronunciations/en/mother-us.mp3",
        sourceUrl: "https://commons.wikimedia.org/w/index.php?curid=1113599",
        license: {
          name: "BY-SA 3.0",
          url: "https://creativecommons.org/licenses/by-sa/3.0",
        },
      },
    ],
    meanings: [
      {
        partOfSpeech: "noun",
        definitions: [
          {
            definition:
              "A person (especially an entomologist) or animal that catches moths.",
            synonyms: [],
            antonyms: [],
          },
        ],
        synonyms: [],
        antonyms: [],
      },
    ],
    license: {
      name: "CC BY-SA 3.0",
      url: "https://creativecommons.org/licenses/by-sa/3.0",
    },
    sourceUrls: [
      "https://en.wiktionary.org/wiki/moth-er",
      "https://en.wiktionary.org/wiki/mother",
    ],
  },
];
